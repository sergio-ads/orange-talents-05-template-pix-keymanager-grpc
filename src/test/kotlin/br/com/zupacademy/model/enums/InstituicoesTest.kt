...
internal class InstituicoesTest {

    @Test
    fun `deve procurar instituicao por codigo`() {
        INSTITUICOES.forEach { key, value -> assertEquals(value, Instituicoes.nome(key)) }
    }

    @Test
    fun `deve retornar erro ao localizar instituicao`() {
        val thrown = assertThrows<IllegalStateException> {
            Instituicoes.nome("0")
        }

        assertEquals("Instituição não encontrada pelo ISPB: 0", thrown.message)
    }
